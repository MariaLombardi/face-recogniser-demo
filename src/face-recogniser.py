...
class FaceRecogniser(yarp.RFModule):
    ...
    def respond(self, command, reply):
        print(command)
        print(command.size())
        print(command.get(0).asString())
        print(command.get(1).asString())
        print(command.get(2).asString())
        #command = command.get(0).asList()
        if command.get(0).asString() == 'quit':
            ...
            self.TRAIN = 1
            reply.addString("Training started.")
            ...
            reply.addString('Init. Files will be saved as ' + self.name_file)
                ...
                if poses and len(poses) <= len(self.labels_set):
                    ...
                    if self.TRAIN == 1 and len(poses) == len(self.labels_set):
                        ...
                        # idx 0: therapist, id 1: child
                            ...
                            # train each 50 samples
                            if len(self.dataset) % (50*2) == 0:
                                trainX = np.squeeze(np.asarray([data[0] for data in self.dataset]))
                                trainy = np.asarray([data[1] for data in self.dataset])
                                ...
                                print('Dataset length: %d ; Accuracy: train=%.3f' % (len(self.dataset), score_train * 100))

                                self.svm_model = model
                                self.normaliser = in_encoder
                                self.encoder = out_encoder

                                if len(self.dataset) > 300*2 and score_train > 0.99:
